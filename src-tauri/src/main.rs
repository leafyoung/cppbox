// CPPBox desktop shell. Embeds the Rust (axum) backend in-process: on startup
// it binds 127.0.0.1:0, serves the API + frontend, then opens a webview to that
// URL. No Python, no sidecar, no port-IPC handshake — one binary.
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![app_port])
