...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/admin/projects/{pid}/feedback",
            get(get_feedback).post(save_feedback),
        )
...
async fn worker_creds(db: &sqlx::SqlitePool) -> (Option<String>, Option<String>) {
    let url = get_setting(db, "worker_url")
        .await
        .or_else(|| std::env::var("CPPBOX_WORKER_URL").ok())
        .filter(|s| !s.is_empty());
    let secret = get_setting(db, "worker_secret")
        .await
        .or_else(|| std::env::var("CPPBOX_WORKER_SECRET").ok())
        .filter(|s| !s.is_empty());
    (url, secret)
}
...
fn parse_student_line(line: &str) -> Result<Option<(i64, String, Option<String>)>, String> {
    ...
    let serial: i64 = serial_str
        .parse()
        .map_err(|_| format!("bad serial: {line:?}"))?;
    let mut email = None;
    let name;
    ...
    let name = if name.is_empty() {
        email
            ...
            .split('@')
            .next()
            .unwrap_or(&format!("student-{serial}"))
            .to_string()
    } else {
        name
    };
...
async fn get_worker_settings(State(st): State<AppState>) -> Json<Value> {
    let url = get_setting(&st.db, "worker_url")
        .await
        .or_else(|| std::env::var("CPPBOX_WORKER_URL").ok())
        .filter(|s| !s.is_empty());
    let secret_set = get_setting(&st.db, "worker_secret")
        .await
        .or_else(|| std::env::var("CPPBOX_WORKER_SECRET").ok())
        .filter(|s| !s.is_empty())
        .is_some();
    Json(
        json!({ "worker_url": url, "worker_secret_set": secret_set, "configured": url.is_some() && secret_set }),
    )
}

async fn put_worker_settings(
    State(st): State<AppState>,
    Json(req): Json<WorkerSettings>,
...
async fn list_classes(State(st): State<AppState>) -> Result<Json<Vec<Value>>, ApiError> {
    ...
    let mut out = Vec::new();
    for c in classes {
        let sc: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM students WHERE class_id = ?")
            .bind(&c.id)
            .fetch_one(&st.db)
            .await
            .unwrap_or(0);
        let ac: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM assignments WHERE class_id = ?")
            .bind(&c.id)
            .fetch_one(&st.db)
            .await
            .unwrap_or(0);
...
async fn get_class(
    ...
    let c = sqlx::query_as::<_, Class>("SELECT * FROM classes WHERE id = ?")
        .bind(&cid)
        .fetch_optional(&st.db)
        .await
        .map_err(db_err)?
        .ok_or_else(|| ApiError(StatusCode::NOT_FOUND, "Class not found".into()))?;
    let students = sqlx::query_as::<_, Student>("SELECT * FROM students WHERE class_id = ?")
        .bind(&cid)
        .fetch_all(&st.db)
        .await
        .map_err(db_err)?;
    let assignments =
        sqlx::query_as::<_, Assignment>("SELECT * FROM assignments WHERE class_id = ?")
            .bind(&cid)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?;
    let student_list: Vec<Value> = students
        .iter()
        .map(|s| {
            json!({
    ...
    meta.as_object_mut()
        .unwrap()
        .insert("student_list".into(), json!(student_list));
    meta.as_object_mut()
        .unwrap()
        .insert("assignment_list".into(), json!(assignment_list));
    Ok(Json(meta))
}
...
async fn delete_class(
    ...
    let exists = sqlx::query_as::<_, Class>("SELECT * FROM classes WHERE id = ?")
        .bind(&cid)
        .fetch_optional(&st.db)
        .await
        .map_err(db_err)?;
    if exists.is_none() {
        return Err(ApiError(StatusCode::NOT_FOUND, "Class not found".into()));
    }
...
async fn create_assignment(
    State(st): State<AppState>,
    Path(cid): Path<String>,
    Json(req): Json<AssignmentCreate>,
) -> Result<Json<Value>, ApiError> {
    let c = sqlx::query_as::<_, Class>("SELECT * FROM classes WHERE id = ?")
        .bind(&cid)
        .fetch_optional(&st.db)
    ...
    let worker_deadline = if policy == "reject" {
        req.expires_ms
    } else {
        None
    };
    let entries: Vec<(String, Option<i64>)> = minted
        .iter()
        .map(|k| (k.clone(), worker_deadline))
        .collect();
...
async fn list_class_keys(
    ...
    let keys =
        sqlx::query_as::<_, SubmissionKey>("SELECT * FROM submission_keys WHERE class_id = ?")
            .bind(&cid)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?;
    let students: HashMap<String, Student> =
        sqlx::query_as::<_, Student>("SELECT * FROM students WHERE class_id = ?")
            .bind(&cid)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?
...
async fn pull_submissions(
    State(st): State<AppState>,
    Path(aid): Path<String>,
) -> Result<Json<Value>, ApiError> {
    let a = sqlx::query_as::<_, Assignment>("SELECT * FROM assignments WHERE id = ?")
        .bind(&aid)
        .fetch_optional(&st.db)
        .await
        .map_err(db_err)?
    ...
    let keys: std::collections::HashSet<String> =
        sqlx::query_scalar::<_, String>("SELECT key FROM submission_keys WHERE assignment_id = ?")
            .bind(&aid)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?
...
async fn organize(
    ...
    if a.root_folder.as_deref().filter(|s| !s.is_empty()).is_none() {
        return Err(ApiError(
            ...
            "Assignment has no root_folder; set one first".into(),
        ));
    }
    let keys =
        sqlx::query_as::<_, SubmissionKey>("SELECT * FROM submission_keys WHERE assignment_id = ?")
            .bind(&aid)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?;
    ...
    Ok(Json(storage::organize_submissions(
        a.root_folder.as_deref().unwrap_or(""),
        &req.zips_folder,
        &key_lookup,
    )))
}
...
async fn update_assignment(
    State(st): State<AppState>,
    Path(aid): Path<String>,
    Json(req): Json<AssignmentRootUpdate>,
) -> Result<Json<Value>, ApiError> {
    let a = sqlx::query_as::<_, Assignment>("SELECT * FROM assignments WHERE id = ?")
        .bind(&aid)
        .fetch_optional(&st.db)
    ...
    let policy = req.late_policy.clone().unwrap_or_else(|| "filter".into());
    sqlx::query(
        "UPDATE assignments SET root_folder = ?, expires_ms = ?, late_policy = ? WHERE id = ?",
    )
    .bind(root)
    .bind(req.expires_ms)
    .bind(&policy)
    ...
    if !keys.is_empty() {
        let entries: Vec<(String, Option<i64>)> =
            keys.into_iter().map(|k| (k, worker_deadline)).collect();
        let (wurl, wsecret) = worker_creds(&st.db).await;
    ...
    Ok(Json(
        json!({ "ok": true, "root_folder": root, "expires_ms": req.expires_ms, "late_policy": policy }),
    ))
}

async fn open_workspace(
    State(st): State<AppState>,
    Json(req): Json<WorkspaceOpen>,
) -> Result<Json<Value>, ApiError> {
    let subs = storage::scan_workspace(&req.root_folder);
    if subs.is_empty() {
        return Err(ApiError(
            StatusCode::BAD_REQUEST,
            "No sub-folders found (or folder missing)".into(),
        ));
    }
    ...
    if let Some(aid) = &req.assignment_id {
        let a = sqlx::query_as::<_, Assignment>("SELECT * FROM assignments WHERE id = ?")
            ...
            .map_err(db_err)?
            .ok_or_else(|| ApiError(StatusCode::NOT_FOUND, "Assignment not found".into()))?;
        let studs = sqlx::query_as::<_, Student>("SELECT * FROM students WHERE class_id = ?")
            .bind(&a.class_id)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?;
        for s in studs {
            if let Some(ser) = s.serial {
                by_serial.insert(ser, s);
            }
        }
        assignment = Some(a);
    }
    ...
    for (name, path) in &subs {
        ...
        let existing = sqlx::query_as::<_, Snippet>("SELECT * FROM snippets WHERE local_path = ?")
            .bind(path)
        ...
        if let Some(a) = &assignment {
            if let Some(stu) = storage::folder_serial(name).and_then(|s| by_serial.get(&s)) {
                let m = sqlx::query_as::<_, Marking>(
                    "SELECT * FROM markings WHERE assignment_id = ? AND student_id = ?",
                )
                .bind(&a.id)
                .bind(&stu.id)
                .fetch_optional(&mut *tx)
                ...
                match m {
                    ...
                    Some(m) if m.project_id.as_deref() != Some(&pid) => {
                        sqlx::query("UPDATE markings SET project_id = ? WHERE id = ?")
                            .bind(&pid)
                            .bind(&m.id)
                            .execute(&mut *tx)
                            .await
    ...
    tx.commit().await.map_err(db_err)?;
    Ok(Json(
        json!({ "opened": projects.len(), "projects": projects }),
    ))
}
...
async fn grid(
    ...
    .await
    .map_err(db_err)?;
    let markings: HashMap<String, Marking> =
        sqlx::query_as::<_, Marking>("SELECT * FROM markings WHERE assignment_id = ?")
            .bind(&aid)
            .fetch_all(&st.db)
            .await
            .map_err(db_err)?
            .into_iter()
            .map(|m| (m.student_id.clone(), m))
            .collect();
    let rows: Vec<Value> = students
        ...
        .collect();
    Ok(Json(
        json!({ "assignment": { "id": a.id, "name": a.name, "root_folder": a.root_folder }, "students": rows }),
    ))
}
...
async fn get_feedback(
    ...
    let m = sqlx::query_as::<_, Marking>("SELECT * FROM markings WHERE project_id = ?")
        .bind(&pid)
        .fetch_optional(&st.db)
        .await
        .map_err(db_err)?;
    let graded = m
        .as_ref()
        .map(|m| m.graded.unwrap_or(false))
        .unwrap_or(false);
    Ok(Json(json!({
...
async fn save_feedback(
    State(st): State<AppState>,
    Path(pid): Path<String>,
    Json(req): Json<FeedbackUpdate>,
) -> Result<Json<Value>, ApiError> {
    let s = fetch_one(&st.db, &pid).await?;
    storage::write_file(&st.root, &pid, lp(&s), "feedback.md", &req.text)
    ...
    if let Some(m) = &m {
        let now = now();
        let graded: i64 = if req.publish {
            1
        } else {
            m.graded.unwrap_or(false) as i64
        };
        let graded_at = if req.publish {
            Some(now.clone())
        } else {
            m.graded_at.clone()
        };
        sqlx::query(
            "UPDATE markings SET score = ?, graded = ?, graded_at = ?, updated_at = ? WHERE id = ?",
        )
        .bind(&req.score)
        .bind(graded)
        .bind(&graded_at)
    ...
    Ok(Json(
        json!({ "ok": true, "graded": graded, "score": score, "published": req.publish }),
    ))
}
...
async fn create_key(
    ...
    sqlx::query("INSERT INTO submission_keys(key,student_name,course,cohort,slot,class_id,student_id,assignment_id,created_at) VALUES (?,?,?,?,?,NULL,NULL,NULL,?)")
        ...
        .execute(&st.db).await.map_err(db_err)?;
    Ok(Json(
        json!({ "key": key, "student_name": req.student_name, "course": req.course, "cohort": req.cohort, "slot": req.slot }),
    ))
}
...
async fn submit(
    ...
    let key_obj = sqlx::query_as::<_, SubmissionKey>("SELECT * FROM submission_keys WHERE key = ?")
        .bind(&req.key)
        .fetch_optional(&st.db)
        .await
        .map_err(db_err)?
    ...
    let prev: Option<i64> =
        sqlx::query_scalar("SELECT MAX(counter) FROM submissions WHERE key = ?")
            .bind(&req.key)
            .fetch_one(&st.db)
            .await
            .map_err(db_err)?;
    let counter = prev.unwrap_or(0) + 1;
    let seq: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM submissions WHERE project_id = ?")
        .bind(&proj.id)
        .fetch_one(&st.db)
        .await
        .map_err(db_err)?;
    let seq = seq + 1;
    let commit_hash = storage::submission_commit(
        &st.root,
        &proj.id,
        lp(&proj),
        seq,
        Some(&key_obj.student_name),
    );
    ...
    let zip_path = storage::create_submission_zip(
        ...
        counter,
        &files,
        &key_obj.student_name,
        &key_obj.course,
        &key_obj.cohort,
        key_obj.slot.unwrap_or(0),
        &proj.title.clone().unwrap_or_default(),
        &submitted_at,
    )
...
fn submission_json(s: &Submission) -> Value {
    let zip = std::path::Path::new(&s.zip_path)
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("")
        .to_string();
...
async fn list_key_submissions(
    State(st): State<AppState>,
    Path(key): Path<String>,
) -> Result<Json<Vec<Value>>, ApiError> {
    let subs = sqlx::query_as::<_, Submission>(
        "SELECT * FROM submissions WHERE key = ? ORDER BY counter DESC",
    )
    .bind(&key)
    .fetch_all(&st.db)
    .await
...
fn detect_ips() -> Vec<String> {
    ...
    ips.into_iter()
        .filter(|ip| !ip.starts_with("127.") && seen.insert(ip.clone()))
        .collect()
}

async fn vscode_info(State(st): State<AppState>) -> Json<Value> {
    let host = std::env::var("CPPBOX_HOST")
        .ok()
        .or_else(|| {
    ...
    Json(
        json!({ "ssh_host": host, "ssh_user": user, "ips": detect_ips(), "projects_root": projects_root }),
    )
}
