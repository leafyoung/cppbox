...
pub fn write_makefile(root: &Path, id: &str, local_path: Option<&str>, std: &str, extra: &str) {
    let d = project_root(root, id, local_path);
    let _ = std::fs::create_dir_all(&d);
    let body = "# Auto-generated by CPPBox\n\
CXX := clang++\n\
STD := {std}\n\
CXXFLAGS := -std=$(STD) {extra} -Wall -Wextra -pedantic\n\
...
/// flags JSON ({werror,sanitize,opt}) → extra clang flags for the Makefile.
pub fn flags_to_extra(flags_json: Option<&str>) -> String {
    let mut out = vec!["-O2".to_string()];
    if let Some(f) = flags_json.and_then(|v| serde_json::from_str::<serde_json::Value>(v).ok()) {
        if f.get("opt").and_then(|o| o.as_str()) == Some("0") {
            out[0] = "-O0".into();
        }
        if f.get("werror").and_then(|w| w.as_bool()) == Some(true) {
            out.push("-Werror".into());
        }
        if f.get("sanitize").and_then(|s| s.as_bool()) == Some(true) {
            out.push("-fsanitize=address,undefined".into());
        }
    }
    out.join(" ")
}
